...
* Small 10 DPU jobs peak at **2 PM**
* Medium 50 DPU jobs peak at **5 PM**
* Large 200 DPU batch jobs run at **10 PM**

Design an optimal DPU allocation strategy.
...
Implemented dynamic DPU scaling, reducing peak capacity needs by **40%** while maintaining SLA through intelligent job scheduling.
...
def get_dpu_utilization(job_name, hours=24):
    ...
    if datapoints:
        ...
        avg_utilization = (
            sum(d['Average'] for d in datapoints)
            / len(datapoints)
        )

        max_utilization = max(
            d['Maximum'] for d in datapoints
        )

        print(
            f"{job_name}: Avg {avg_utilization:.1f}, "
            f"Max {max_utilization:.1f} DPU"
        )
...
**Profile baseline → Find minimum DPU → Monitor utilization → Implement scheduling → Reserve capacity → Test peaks**

---

## Expert Tip

Use a **30% DPU utilization target** for bursty workloads.

Implement job chaining to reduce peak concurrency.

Monitor shuffle metrics to identify over-allocation.
