...
**Scenario:**
Athena queries run against **10 years of daily partitioned data (3,650+ partitions)**. The crawler adds partitions slowly, impacting query performance and partition discovery.

**Goal:** Improve query performance and partition discovery.

---

## Solution

* Enable Glue partition projection in the Athena table
...
Enabled partition projection, reducing Athena query planning time from **45 seconds to less than 2 seconds** on **10TB+ datasets**.

Query cost was reduced by **30%**.
...
print(
    f"Query creation query: "
    f"{response['QueryExecutionId']}"
)

# Alternative: Update table properties via Glue API

def enable_partition_projection(
    database_name,
    table_name
):
    ...
    print(
        f"Partition projection enabled for "
        f"{database_name}.{table_name}"
    )


enable_partition_projection(
    'analytics_db',
    'logs_partitioned_projected'
)
...
TBLPROPERTIES (
    ...
    'projection.service_name.values' =
    'web-api,mobile-app,batch-service,analytics-service',
...
* Missing digit padding for sequential partition keys

---

## Projection Approach

**Analyze partition structure → Choose projection type (date/enum) → Define ranges → Enable projection → Test coverage**

---

## Expert Tip

* Use projection for time-series data with daily or hourly partitions.
* Verify that projection covers all existing partitions.
* Combine projection with partition filtering in WHERE clauses for optimal performance.
* Monitor Athena query costs after enabling partition projection.
